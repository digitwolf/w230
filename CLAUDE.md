...
`docs/` holds the verified findings: start with `docs/bringup-learnings.md` (live-verified facts, disproven beliefs, architecture) and `docs/kds-protocol.md` (wire-level ECU protocol with captured frames). `docs/hardware.md` is the wiring and parts reference; `docs/toolchain.md` covers setup, flashing and debugging; `docs/ble-protocol.md` the GATT service the iOS app uses; `docs/ota.md` the update pipeline. `ios/` is the SwiftUI companion app: an xtool SwiftPM package built, signed and installed on the iPhone **from this Linux machine** via `ios/scripts/deploy.sh` (see `ios/README.md`; the shared Claude agent `ios-xtool` in `~/.claude/agents/` knows the workflow and constraints — never `@State`, it is a macOS-only macro in the iOS 27 SDK). `project.yml` (XcodeGen) is kept for Mac users and the macOS CI job. `infra/` is the CloudFormation template for the S3+CloudFront update endpoint.
...
- OTA memory: BLE + WiFi + TLS together leave ~20–30 KiB during a download. Keep `CONFIG_MBEDTLS_DYNAMIC_BUFFER`, the small WiFi buffer pools and the sequential-erase `SlotWriter` in `ota.rs`; the binding's `initiate_update` erases the whole slot and starved the watchdog. Rollback (PENDING_VERIFY → VALID after the 20 s self-test) is verified on hardware; `flash.sh` erases `otadata` so a USB-flashed board starts clean.
- Xtensa toolchain trap (seen 2026-09-22): a function taking six-plus argument words received uninitialised stack (0xA5A5A5A5) for its trailing `&str` arguments. Keep ESP-side function signatures short; bundle extra parameters in a struct or read crate constants.
- Debugging panics: the release profile keeps line tables; resolve backtrace addresses with `xtensa-esp32-elf-addr2line -e target/xtensa-esp32-espidf/release/w230-gear-indicator -f -i -C <addr>` (binary under `~/.rustup/toolchains/esp/xtensa-esp-elf/*/xtensa-esp-elf/bin/`).
