...
`docs/` holds the verified findings: start with `docs/bringup-learnings.md` (live-verified facts, disproven beliefs, architecture) and `docs/kds-protocol.md` (wire-level ECU protocol with captured frames). `docs/hardware.md` is the wiring and parts reference; `docs/toolchain.md` covers setup, flashing and debugging.
...
- `esp32/` (`w230-gear-indicator`) — the flashable binary: UART transport + fast-init timing (`kds`), NVS persistence (`learn_store`), WiFi softAP + HTTP dashboard (`web`), poll loop (`main`).
...
Compile-gated diagnostics in main.rs: `WIFI_DIAG` (softAP `W230-GEAR` / `w230diag`, http://192.168.71.1/ — live status JSON, histogram CSV, calibration wipe; off by default because WiFi interrupts glitch the WS2812 timing), `DIAG_SCAN`, `DEEP_SCAN`, `DEMO_MODE`. On-bike controls: button short press = brightness, 3 s hold = wipe calibration.
